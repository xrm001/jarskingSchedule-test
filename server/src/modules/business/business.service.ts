  ...
  async createPersonalSchedule(actor: AuthenticatedUser, body: JsonObject, bossSpace?: string) {
    ...
    try {
      ...
      if ((error as { code?:string }).code === '23P01') {
        const conflict = await this.database.query<{ title:string|null; start_at:Date; end_at:Date }>(
          `SELECT title,start_at,end_at FROM schedule_entries
           WHERE boss_user_id=$1 AND status='ACTIVE'
        ...
        const detail = existing
          ...
          : '已有日程';
        throw new ConflictException({ code:'SCHEDULE_CONFLICT', message:`该时间与${detail}重叠，请调整日期或时间` });
      }
      throw error;
    }
  }
