...
const bossSpaces = { resolveBossId: vi.fn().mockResolvedValue('boss-1') } as unknown as BossSpaceService;

describe('SessionService', () => {
  it('sanitizes an external OAuth return path', async () => {
    const query = vi.fn().mockResolvedValue({ rows:[], rowCount:1 });
    const sessions = new SessionService({ query } as unknown as DatabaseService, bossSpaces);
